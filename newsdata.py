...
def top_viewed_article():
    ...
    execute_query("""SELECT DISTINCT articles.title, t1.views FROM articles, log,
                    ...
                    WHERE articles.slug = t1.path ORDER BY views DESC
                    LIMIT 3;""",
                  "\nQuery 1 RESULT: What are the most " +
                  "popular three articles of all time?")


def top_authors():
    """Define new method for getting Query 2: Top Author name
        and their total views"""
    execute_query("""SELECT name, sum(views) AS views FROM author_n_view
...
def error_days():
    """define new funtion for getting Query 3:
        Day where most error has occured"""
    # Execute SQL Query
    execute_query("""SELECT * FROM (SELECT error_day.day,
                    ROUND(SUM(error_day.status_count) /
                    SUM(total_request.status_count) * 100, 2) AS percent
                    FROM error_day INNER JOIN total_request ON
                    error_day.day = total_request.day GROUP BY error_day.day,
                    total_request.status_count ORDER BY percent DESC) AS t1
                    WHERE t1.percent > 1;""",
